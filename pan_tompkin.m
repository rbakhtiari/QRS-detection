function [qrs_amp_raw,qrs_i_raw,delay,ecg_m,ecg_h]=pan_tompkin(ecg,fs,gr,subjectid)
...
if gr
figure,az(1)=subplot(311);plot(ecg_h);title([num2str(subjectid),': QRS on Filtered Signal']);axis tight;
...
line(repmat(qrs_i_raw,[2 1]),repmat([min(ecg-mean(ecg))/2; max(ecg-mean(ecg))/20],size(qrs_i_raw)),'LineWidth',.1,'LineStyle','-.','Color','r');
linkaxes(az,'x');
zoom on;
end
end